...
export default class User$ extends $ implements F {
  constructor(
    ...
    u.findOne({ where: { account: "admin" } }).then(v => {
      if (v === undefined) {
        u.save(new User({
          account: "admin", pwd: encrypt("654321", "shake256", "latin1", 50)
        } as User)).then(user => { console.log("User has been saved:", user); })
      }
      else console.error('\x1B[33;1m%s\x1B[22m', "董事长已存在!\x1B[37m"); return;
    })//创建默认账户admin
  }

  async register(user: User): Promise<any> {
    const exist = await this.u.findOne({ where: { account: user.account } });
