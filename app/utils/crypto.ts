...
// 预计算字符索引，把 NTo10 中的 O(n²) 字符查找降为 O(1)
const cIndex: Map<string, number> = new Map();
for (let _i = 0; _i < c.length; ++_i) cIndex.set(c[_i], _i);
...
const TenToN = (v: number, n: number): string => {
  ...
  let num = "", mod
  for (let i = 1; i < 54; ++i) {
    mod = v % n; v = Math.floor(v / n);
    if (v > 0) {
      num = c.charAt(mod) + num
 ...
 * @param n 表示v是多少进制
 */
const NTo10 = (v: string, n: number): number => {
  if (n > 66 || n < 2) return; if (n < 37) v = v.toLowerCase();
  let addNumber = 0, l = v.length
  for (let i = 0; i < l; ++i) {
    const h = cIndex.get(v.charAt(l - i - 1));
