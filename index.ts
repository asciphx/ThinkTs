...
createConnection().then(async connection => {Tag.Init(connection.name);//Require to use decorator preprocessing
  ...
  .use(async (ctx, next) => {
    ...
    if(token){
      try {
        Jwt.verify(token.split(' ')[1],Number("0x"+ctx.headers.secret).toString(36),{complete:true});await next();
      } catch (e) {
  ...
  })//动态secret,基于前后端共识算法，采用二次转，62进制转16进制+后端补码0x再转36进制(因账户名只能是数字或者英文字母共62个)。
